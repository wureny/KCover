...
export function createKCoverWorker() {
  ...
  const workerCode = `
    ...
    // 枚举所有k-子集的位掩码
    function enumSubsets(n, k) {
      const result = [];
      
      function backtrack(start, mask, count) {
        if (count === k) {
          result.push(mask);
          return;
        }
    ...
    // 构建覆盖表 - 记录每个k-子集覆盖了哪些j-子集
    function buildCoverageTable(kMasks, jMasks, s, minGroups) {
      const coverageTable = Array(kMasks.length).fill(null).map(() => []);
    ...
    // 评估方案质量
    function evaluateSolution(coverageTable, chosenIndices, jMasksCount, minGroups = 1) {
      // 检查是否覆盖了所有j-子集
      const covered = new Array(jMasksCount).fill(false);
      for (const idx of chosenIndices) {
        for (const jIdx of coverageTable[idx]) {
    ...
    // 移除冗余覆盖
    function pruneRedundant(chosen, coverageTable, jMasksCount, minGroups = 1) {
      // 如果选择的组数小于或等于最小要求，则不进行冗余移除
      if (chosen.length <= minGroups) return;
    ...
    // 贪心算法求解最小覆盖问题
    function greedyCover(kMasks, jMasks, coverageTable, minGroups = 1) {
      const chosen = [];
      const covered = new Array(jMasks.length).fill(false);
      let uncoveredCount = jMasks.length;
    ...
    // 速度模式入口函数 - 贪心算法
    function solveWithSpeedMode(params) {
      // 预处理
      const { kMasks, jMasks, coverageTable } = preprocess(params);
    ...
    // 精准模式入口函数 - 基于局部搜索的精确求解
    function solveWithAccurateMode(params) {
      // 对于minGroups > 1的情况，使用改进的贪心算法
      if (params.minGroups > 1) {
        // 预处理
        const { kMasks, jMasks, coverageTable } = preprocess(params);
      ...
      // 对于传统的最小覆盖问题(minGroups=1)，使用局部搜索
      const { kMasks, jMasks, coverageTable } = preprocess(params);
